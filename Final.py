# -*- coding: utf-8 -*-
"""
Created on Wed May  4 14:47:56 2022

@author: Bro-teins Group
...
class Compear():
    ...
    def extend_final_idx(self):
        for i in range(self.df.shape[0]):
            ...
            while (f < len(self.str1)):
                ...
                if (self.str1_n[oidx:f] == self.str2_n[iidx:g]):
                    self.df.loc[i, "Fidx"] = g - 1
                    self.df.loc[i, "Origfidx"] = f - 1
                    self.df.loc[i, "Match"] = self.str1_n[oidx:f]
                    f += 1
                    if g < len(self.str2_n):
                        g += 1
                else:
                    break
    ...
    def Word_document_init(self, doc):
        ...
        mol_weight_str1 = doc.add_paragraph(
            "Molecular Weight of Original Sequence: " + str(self.get_molecular_weight(self.str1))
        )
        para0 = doc.add_paragraph(
            "Comparison Sequence: " + self.str2
        )
        mol_weight_str2 = doc.add_paragraph(
            "Molecular Weight of Comparison Sequence: " + str(self.get_molecular_weight(self.str2))
        )
...
class GUI():
    def __init__(self):
        ...
        #entery to string 
        #self.String1= self.String1.get()
        #self.String2= self.String2.get()
    ...
    def proteincompare(self):  # function to compare strings update bar and call msg finish
        ...
        for i in unaccepted_char:                               #Iterate through string to find if there is unaccepted char
            ...
            if (x == True or y == True):
                ...
                break
        String1 = self.String1.get().replace(" ","").replace("\n","")
        String2 = self.String2.get().replace(" ","").replace("\n","")
        if (String1.isalpha() == False or String2.isalpha() == False):
